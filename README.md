...
当前项目约定的运行镜像名为 `yupentestpilot/sandbox:latest`。
构建命令：

```bash
docker build -f docker/sandbox.Dockerfile -t yupentestpilot/sandbox:latest .
```

`docker/sandbox.Dockerfile` 会在构建时直接从上游官方 release 拉取附加工具，不依赖本地缓存目录。

代码会直接调用这个镜像，不再依赖环境变量切换运行镜像。
