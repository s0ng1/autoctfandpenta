...
- 主入口 `YuPentestPilot.py` 已切到 intent-native 初始化路径
  ...
  - 运行前会先 bootstrap runtime metadata / artifacts / template
...
- 已将 pentest 报告主路径切到 Word 模板输出
  - 最终报告强制为 `.docx`
  - 模板文件为 `docs/渗透测试报告模板V1.2.docx`
  - 运行时复制到 workspace 内供容器使用
...
2. 运行前 bootstrap `intentlang/metadata`、`intentlang/artifacts`、报告模板
3. 容器内 agent 先读取：
   - `run`
   - `strategy`
   - `intents`
...
5. 验证阶段写入：
   ...
   - `candidate_evidence`
   - `verified_findings`
6. 报告阶段优先从 `verified_findings` 直接生成模板化 Word 报告
...
- 已有默认值与枚举约束
- `verified_findings` 已支持模板报告需要的字段：
  - `control_point`
  - `evaluation_unit`
  - `risk_analysis`
  - `vuln_url`
